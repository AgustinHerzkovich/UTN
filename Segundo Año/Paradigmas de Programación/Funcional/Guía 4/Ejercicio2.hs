...
promedioFrecuenciaCardiaca :: (Fractional a) => [Integer] -> a
promedioFrecuenciaCardiaca frecuenciaCardiaca = fromIntegral (sum frecuenciaCardiaca) / fromIntegral (length frecuenciaCardiaca)

conocerMuestra :: (Integral a) => a -> a
conocerMuestra m = m `div` 10

frecuenciaCardiacaMinuto :: Int -> Integer
frecuenciaCardiacaMinuto = (frecuenciaCardiaca !!) . (`div` 10)

frecuenciasHastaMomento :: Int -> [Integer]
frecuenciasHastaMomento m = take (fromIntegral (conocerMuestra m) + 1) frecuenciaCardiaca

{-
Durante un entrenamiento físico de una hora, cada 10 minutos de entrenamiento se
tomó la frecuencia cardíaca de uno de los participantes obteniéndose un total de 7 muestras que son las siguientes:
frecuenciaCardiaca = [80, 100, 120, 128, 130, 123, 125]
